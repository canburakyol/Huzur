...
export interface WordByWordDataMap {
  [surahNumber: number]: SurahWordData;
}

export const wordByWordData: WordByWordDataMap = {
  1: {
    name: "Fatiha",
    arabicName: "الفاتحة",
    meaning: "Açılış",
...
export const getWordByWordData = (surahNumber: number): SurahWordData | null => {
  return wordByWordData[surahNumber] || null;
};

export const getFreeSurahs = (): number[] => {
  return Object.keys(wordByWordData).map(Number);
};

export const hasWordByWordData = (surahNumber: number): boolean => {
