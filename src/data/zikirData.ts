...
export interface DhikrCategoryWithCount extends DhikrCategory {
  count: number;
}
...
export const DHIKR_LIST: Dhikr[] = [
  ...
  },
  {
    ...
    meaning: 'Allah\'ım beni önümden, arkamdan, sağımdan, solumdan, üstümden koru. Azametimle altımdan gelecek tehlikeden sığınırım',
...
export const getDhikrByCategory = (categoryId: string): Dhikr[] => {
  return DHIKR_LIST.filter(d => d.category === categoryId);
};

export const getCategoriesWithCounts = (): DhikrCategoryWithCount[] => {
...
export const getDailyRecommended = (): Dhikr[] => {
  const dailyIds = [1, 2, 3, 5, 6, 7];
