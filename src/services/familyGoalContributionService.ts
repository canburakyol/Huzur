...
const getContributionMap = (): Record<string, string> => storageService.getItem(AUTO_CONTRIBUTION_KEY, {});

const markContribution = (key: string, dateKey: string): void => {
  const history = getContributionMap();
  history[key] = dateKey;
  storageService.setItem(AUTO_CONTRIBUTION_KEY, history);
};

const hasContributionForToday = (key: string, dateKey: string): boolean => {
  const history = getContributionMap();
  return history[key] === dateKey;
};

export const contributeFamilyGoalOncePerDay = async (triggerKey: string, contributionType = 'automatic'): Promise<boolean> => {
  ...
  try {
    ...
    logger.warn('[FamilyGoalContribution] Skipped automatic contribution', (error as Error)?.message || error);
    return false;
  }
};
