...
export const hydrateLocalIntelligenceState = async () => {
  if (hydrated) {
    return cachedState;
  }
...
const updateCachedState = (updater) => {
...
const getMostActiveSlot = (pattern = {}) => {
  ...
  return ranked[0]?.[0] || 'morning';
};

const deriveRiskBand = (context = {}) => {
...
const getGoalWeights = (primaryGoal) => {
...
const scoreCandidate = ({ candidateId, context = {}, slot, state, riskBand }) => {
  ...
  if (FEATURE_TO_MODULE[lastFeature] === candidateId) {
    score += 16;
  }
  ...
  // Exposure decay: modules seen too often get penalized (diminishing returns)
...
const buildHeadline = (topModule, riskBand) => {
...
const buildExplanation = (topModule, context = {}, slot) => {
  const streak = Number(context?.streak?.current) || 0;
  const activeDays = Number(context?.weeklySnapshot?.activeDays) || 0;
  const slotCopy = {
...
const buildSocialHint = (context = {}, topModule) => {
...
const buildFallbackRanking = (candidates = []) => ({
