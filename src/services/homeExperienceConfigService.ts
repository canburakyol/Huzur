...
type HomeExperienceConfig = {
  ...
  priorityCardEnabled: boolean;
  recoveryCardEnabled: boolean;
  aiRecommendationEnabled: boolean;
  featureGridEnabled: boolean;
  dailyContentEnabled: boolean;
  dailyQuestsEnabled: boolean;
};

export const DEFAULT_HOME_EXPERIENCE_CONFIG: HomeExperienceConfig = Object.freeze({
  enabled: true,
  source: 'default',
  variant: 'recovery_v1',
...
const VARIANT_PRESETS: Record<string, Partial<HomeExperienceConfig>> = {
  recovery_v1: {},
  classic_safe: {
...
const normalizeVariant = (value: string | undefined, experimentVariant: string): string => {
  if (['recovery_v1', 'classic_safe', 'minimal_safe'].includes(value || '')) return value as string;
  return experimentVariant === 'B' ? 'classic_safe' : DEFAULT_HOME_EXPERIENCE_CONFIG.variant;
};

export const normalizeHomeExperienceConfig = (value: Record<string, unknown> = {}, experimentVariant = 'A'): HomeExperienceConfig => {
  const source = value && typeof value === 'object' ? value : {};
  const enabled = normalizeBoolean(source.enabled, DEFAULT_HOME_EXPERIENCE_CONFIG.enabled);
  const variant = enabled
    ? normalizeVariant(source.variant as string | undefined, experimentVariant)
    : 'minimal_safe';
  const preset = VARIANT_PRESETS[variant] || {};
  const base: HomeExperienceConfig = {
    ...DEFAULT_HOME_EXPERIENCE_CONFIG,
    ...preset,
    source: (source.source as string) || DEFAULT_HOME_EXPERIENCE_CONFIG.source,
    enabled,
    variant,
    experimentVariant,
  } as HomeExperienceConfig;
...
const parseRemoteJson = (rawValue: string | null): Record<string, unknown> | null => {
...
export const loadHomeExperienceConfig = async (): Promise<HomeExperienceConfig> => {
