...
interface HomeRankingPayload {
  context?: Record<string, unknown>;
  candidates?: Array<{ id: string }>;
}

interface HomeRankingResult {
  provider: string;
  headline: string;
  explanation: string;
  socialHint: string;
...
interface WeeklyInsightsPayload {
  context?: Record<string, unknown>;
  weekKey?: string;
  [key: string]: unknown;
}

interface PushHintPayload {
  type?: string;
  context?: Record<string, unknown>;
...
const callAiFunction = async (name: string, payload: Record<string, unknown> = {}): Promise<unknown> => {
...
const buildAiIncidentMetadata = (payload: Record<string, unknown> = {}): AiIncidentMetadata => ({
...
export const getAssistantSessionState = (): AssistantSessionState => ({
...
export const saveAssistantSessionState = (response: AssistantResponse | null): void => {
...
export const askAssistantV2 = async (payload: AssistantPayload = {}): Promise<AssistantResponse | null> => {
  try {
    ...
    recordAiIncident('assistant', 'callable_failed', error as Error, buildAiIncidentMetadata(payload));
    logger.warn('[AIService] askAssistantV2 failed', error);
    return null;
  }
};

export const getHomeRankingV2 = async (payload: HomeRankingPayload = {}): Promise<HomeRankingResult> => {
  try {
    return await getLocalHomeRanking(payload) as HomeRankingResult;
  } catch (error) {
    recordAiIncident('home_ranking', 'local_heuristic_failed', error as Error, buildAiIncidentMetadata(payload));
...
export const generateWeeklyInsightsV1 = async (payload: WeeklyInsightsPayload = {}): Promise<unknown> => {
  try {
    return await callAiFunction('generateWeeklyInsightsV1', payload);
  } catch (error) {
    recordAiIncident('weekly_insight', 'callable_failed', error as Error, buildAiIncidentMetadata(payload));
...
export const getPersonalizedPushHintsV1 = async (payload: PushHintPayload = {}): Promise<PushHintResult> => {
  try {
    return await getLocalPushHint(payload) as PushHintResult;
  } catch (error) {
    recordAiIncident('push_hint', 'local_heuristic_failed', error as Error, buildAiIncidentMetadata(payload));
