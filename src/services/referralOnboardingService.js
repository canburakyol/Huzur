...
const buildStep = (id, label, description, status) => ({
  id,
  label,
  description,
  status,
...
export const buildReferralOnboardingPlan = ({
  ...
  const rewardReady = Boolean(progress?.rewards?.inviteeUnlockedAt);
  const syncIssue = progress?.server?.inviteeSummary?.syncIssue || '';
...
export const buildReferralOnboardingAnalyticsPayload = (plan = {}, extra = {}) => ({
  ...(plan?.invitedByCode ? {
  referred_user: true,
