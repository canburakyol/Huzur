...
type ToleranceMap = Record<string, number>;
...
type PrayerValidationResult = {
  severity: string;
  reason?: string;
  perPrayerDelta: Record<string, number | null>;
  recommendedAction: string;
};

const toMinutes = (timeStr: string | undefined | null): number | null => {
...
const diffInMinutes = (a: string | undefined, b: string | undefined): number | null => {
...
const getToleranceMap = (countryCode = 'DEFAULT'): ToleranceMap => {
  const normalized = String(countryCode || 'DEFAULT').toUpperCase();
  return (PRAYER_TOLERANCE_MINUTES as Record<string, ToleranceMap>)[normalized] || PRAYER_TOLERANCE_MINUTES.DEFAULT;
};

export const validatePrayerTimings = ({ primary, secondary, countryCode = 'DEFAULT' }: ValidationInput): PrayerValidationResult => {
  ...
  const perPrayerDelta: Record<string, number | null> = {};
  let hasWarn = false;
  ...
  const result: PrayerValidationResult = {
    severity,
    perPrayerDelta,
...
export const getLastPrayerValidation = (): unknown => {
  return storageService.getItem(STORAGE_KEYS.PRAYER_VALIDATION_LAST, null);
};

export const saveLastSafePrayerTimings = (timings: unknown): void => {
...
export const getLastSafePrayerTimings = (): unknown => {
