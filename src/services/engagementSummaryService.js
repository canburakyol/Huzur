...
export const getRecentDateKeys = (days = 7, anchor = new Date()) => {
...
export const getXpHistory = () => storageService.getItem(XP_HISTORY_KEY, []);

export const recordXpEvent = ({ baseAmount = 0, appliedAmount = 0, source = 'general', at = new Date().toISOString() }) => {
...
export const getDailyQuizHistory = () => storageService.getItem(QUIZ_HISTORY_KEY, {});

export const getDailyQuizResult = (dateKey = toLocalDateKey()) => {
  const history = getDailyQuizHistory();
  return history[dateKey] || null;
};

export const saveDailyQuizResult = ({ score = 0, totalQuestions = 0, completedAt = new Date().toISOString() }) => {
  const history = getDailyQuizHistory();
...
export const getDiscoveryHistory = () => storageService.getItem(DISCOVERY_HISTORY_KEY, {});

export const recordDiscoveryView = (count = 1, viewedAt = new Date().toISOString()) => {
...
export const getDailyTaskHistory = () => storageService.getItem(DAILY_TASK_HISTORY_KEY, {});

export const saveDailyTasksSnapshot = ({ dateKey = toLocalDateKey(), tasks = [] }) => {
...
const countUniqueActivityDays = (entries, recentKeys) => {
  const days = new Set();
...
export const buildWeeklyEngagementSnapshot = (days = 7, anchor = new Date()) => {
  ...
  const routineHistory = storageService.getItem(ROUTINE_HISTORY_KEY, {});
...
export const getWeekStartDate = (anchor = new Date()) => {
  const base = startOfDay(anchor) || new Date();
...
export const getCurrentWeekKey = (anchor = new Date()) => {
  const weekStart = getWeekStartDate(anchor);
  return toLocalDateKey(weekStart);
};

export const isDateInCurrentWeek = (value, anchor = new Date()) => {
