...
// recommendationEngine.js was removed (dead code). Inline minimal fallback:
const getPersonalizedSuggestions = () => ({ suggestions: [], context: '' });

/**
 * Core state + logic hook for the AI Assistant feature.
 * Extracted from the monolithic Assistant.jsx (766 lines).
 */
const useAssistant = ({
  streakData,
  dailyContent,
  ...
  const getFaqAnswer = useCallback((query) => getAssistantFaqAnswer(query, undefined), []);
  ...
  const formatAssistantReply = useCallback(
    (result, query) => {
      ...
      if (result.safeModeNotice) parts.push(result.safeModeNotice);
  ...
  useEffect(() => {
    ...
    // Will be overridden in shell with t() call
  }, [getNextMessageId]);
