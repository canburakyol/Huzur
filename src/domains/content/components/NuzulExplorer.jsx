...
const NuzulExplorer = ({ onClose }) => {
  ...
  // Örnek sorular
  const suggestedQuestions = t('nuzul.suggestedQuestions', { returnObjects: true, defaultValue: [
  ...
  const handleSend = (text = query) => {
    ...
    const localResult = i18n.language === 'tr' ? searchNuzul(text) : null;
    ...
    setTimeout(() => {
      if (localResult) {
        ...
        setMessages(prev => [...prev, { 
          role: 'assistant', 
          content: t('nuzul.noResult', 'Bu ayet/sure hakkında veritabanımızda henüz detaylı nüzul bilgisi bulunamadı. Lütfen "Alaq Suresi", "Fatiha" veya "Ayetel Kürsi" gibi daha genel başlıkları denemeyi unutma.'),
          timestamp: new Date() 
        }]);
      }
      setIsLoading(false);
    ...
    <div className="nuzul-explorer-container">
      ...
      <div className="nuzul-header">
        ...
        <div className="header-content">
          <h1>📖 {t('nuzul.title', 'Nüzul Sebebi')}</h1>
          <p className="subtitle">{t('nuzul.subtitle', 'Ayetlerin iniş sebeplerini öğrenin')}</p>
        </div>
      </div>
      ...
      <div className="nuzul-messages">
        {messages.length === 0 ? (
          <div className="nuzul-welcome">
            ...
            <h2>{t('nuzul.explorerTitle', 'Nüzul Sebebi Gezgini')}</h2>
            <p>{t('nuzul.explorerDesc', 'Ayetlerin hangi olay üzerine indirildiğini öğrenmek için soru sorun.')}</p>
            
            <div className="suggested-questions">
              <h4>{t('nuzul.exampleQuestions', 'Örnek Sorular:')}</h4>
                ...
                <div className="message-content">
                  ...
                  {msg.source && (
                    <div className="source-tag">
                      {t('common.source', 'Kaynak')}: {msg.source}
                    </div>
                  )}
                </div>
              </div>
