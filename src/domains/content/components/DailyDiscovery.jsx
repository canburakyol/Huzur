    ...
    <div className="reveal-stagger" style={{ margin: '20px' }}>
      ...
      <div style={{
        ...
        {cards.map((card, index) => {
          ...
          const useTurkishFallback = i18n.language === 'tr';
          const cardLabel = t(`discovery.cards.${card.id}.label`, {
            defaultValue: useTurkishFallback ? card.label : t(`discovery.typeLabels.${card.type}`, card.type)
          });
          const cardTitle = t(`discovery.cards.${card.id}.title`, {
            defaultValue: useTurkishFallback ? card.title : t('discovery.cardFallbackTitle', 'Open today’s discovery')
          });
          const cardSource = t(`discovery.cards.${card.id}.source`, {
            defaultValue: useTurkishFallback ? card.source : t('discovery.cardFallbackSource', 'Huzur')
          });
