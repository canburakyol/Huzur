...
export function useStreakGuards() {
  ...
  const [streak24hRecovery, setStreak24hRecovery] = useState<Streak24hRecovery | null>(null);
  ...
  const handleConfirm24hRecovery = (): void => {
  ...
  const handleRewarded24hRecovery = async (): Promise<RewardedAdResult> => {
    if (isPro()) {
    ...
    try {
      ...
      return { success: false, error: (error as Error)?.message || "Rewarded recovery failed" };
    }
  };

  const handleUseProtectionToken = (): void => {
