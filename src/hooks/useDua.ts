...
interface UseDuaResult {
  loading: boolean;
  error: string | null;
  duas: Dua[];
  userId: string | null;
  prayedDuaIds: Set<string>;
  submittingDuaIds: Set<string>;
  createDua: (text: string, isAnonymous: boolean, authorName?: string) => Promise<boolean>;
  prayForDua: (duaId: string) => Promise<{ success: boolean; alreadyPrayed?: boolean; aminCount?: number }>;
  refreshDuas: () => Promise<void>;
}

const mergeDuasWithSeeds = (duaList: Dua[] = []): Dua[] =>
  [...DUA_DISCOVERY_SEEDS, ...duaList]
    .reduce((acc: Dua[], dua) => {
...
export const useDua = (): UseDuaResult => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [duas, setDuas] = useState<Dua[]>(() => mergeDuasWithSeeds());
  const [userId, setUserId] = useState<string | null>(null);
  const [authLoading, setAuthLoading] = useState(true);
  const [prayedDuaIds, setPrayedDuaIds] = useState<Set<string>>(() => new Set());
  const [submittingDuaIds, setSubmittingDuaIds] = useState<Set<string>>(() => new Set());

  const loadRecentDuas = async (): Promise<void> => {
    try {
  ...
  useEffect(() => {
    ...
    const fetchPrayedDuas = async () => {
      try {
        ...
        const snapshot = await getDocs(collection(database, "users", userId, "duaAmins"));
        if (!isMounted) return;
  ...
  const createDua = async (text: string, isAnonymous: boolean, authorName?: string): Promise<boolean> => {
    try {
  ...
  const prayForDua = async (duaId: string): Promise<{ success: boolean; alreadyPrayed?: boolean; aminCount?: number }> => {
    ...
    try {
      ...
      if (typeof result?.aminCount === "number") {
        setDuas((current) =>
          current.map((dua) => (dua.id === duaId ? { ...dua, aminCount: result.aminCount } : dua))
        );
      }
      return result;
    } catch (err) {
