...
class PrayerDataSyncWorker(
    ...
    companion object {
        ...
        private const val KEY_LAST_IMMEDIATE_ENQUEUE_AT = "last_immediate_enqueue_requested_at"
        ...
        private val IMMEDIATE_ENQUEUE_DEBOUNCE_MS = TimeUnit.MINUTES.toMillis(30)
        ...
        fun enqueueImmediateIfStale(context: Context) {
            ...
            if (now - lastImmediateEnqueueAt < IMMEDIATE_ENQUEUE_DEBOUNCE_MS) {
                return
            }

            prefs.edit()
                .putLong(KEY_LAST_IMMEDIATE_ENQUEUE_AT, now)
                .apply()
